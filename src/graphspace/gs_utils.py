...
def grid_layout(G, graph_attr, parent_coord=None, padding=35, node_size=40):
    ...
    sorted_terms = sorted(nodes_per_parent, key=lambda k: len(nodes_per_parent[k]))
    # first add the sources, last add the targets
    if 'Sources' in sorted_terms:
        # remove sources and targets from the rest
        sorted_terms = [t for t in sorted_terms if t not in ['Sources', 'Targets']]
        middle_x = term_col_size / 2
        nodes = nodes_per_parent['Sources']
        # to make it mostly square, take the square root of the number of terms
        num_cols = np.ceil(len(nodes)**(1/2))
        start_middle_x = middle_x-(num_cols/2)
        layout, x, y = put_nodes_in_square(
            layout, nodes, node_size, padding, num_cols, start_middle_x, start_middle_x, row_y)
        row_y = y + 120

    for i, parent in enumerate(sorted_terms):
        nodes = nodes_per_parent[parent]

        # to make it mostly square, take the square root of the number of nodes
        num_cols = np.ceil(len(nodes)**(1/2))
        if parent_coord is not None and parent in parent_coord:
            ...
            y = row_y
        layout, x, y = put_nodes_in_square(
            layout, nodes, node_size, padding, num_cols, col_x, x, y)
        if y > largest_y:
            largest_y = y

        # now move to the next column 
        col_x += num_cols * (node_size + padding) + 250
        y = row_y
        # if we've reached the end of the row, start the next row
        if col_x > term_col_size:
            col_x = start_x
            row_y = largest_y + 180
            largest_y = 0

    if 'Targets' in nodes_per_parent:
        row_y = largest_y + 120 if largest_y != 0 else row_y
        # now add the Targets
        middle_x = term_col_size / 2
        nodes = nodes_per_parent['Targets']
        # to make it mostly square, take the square root of the number of nodes
        num_cols = np.ceil(len(nodes)**(1/2))
        start_middle_x = middle_x-(num_cols*(node_size + padding)/2)
        layout, x, y = put_nodes_in_square(
            layout, nodes, node_size, padding, num_cols, start_middle_x, start_middle_x, row_y)
