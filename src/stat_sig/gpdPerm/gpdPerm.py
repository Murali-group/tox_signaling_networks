...
def est(x0,y,Nexcmax=250,alpha=0.05,method='ML'):
    ...
    Nexc = np.nan
    good_fit_p = 0
    # TODO 10 seems arbitrary to me. SHouoldn't it scale with the size of your dataset?
    if M>=10:
        p = float(M)/float(N)
    else:
...
#            Nexc = Nexcvec[count]
#            p, k, a, good_fit_p = pgpdML(y,x0,float(N),Nexc,Padth=alpha)
#            count = count+1
        highest_p = 0
        highest_p_vars = [np.nan]*6 
        while good_fit_p < alpha and count<LNV:
            Nexc = Nexcvec[count]
            p, k, a, good_fit_p = pgpdML(y,x0,float(N),Nexc,Padth=alpha)
            if p > highest_p:
                highest_p = p
                highest_p_vars = [p, M, k, a, Nexc, good_fit_p] 
            count = count+1
        if good_fit_p > alpha:
            if p > highest_p:
                print("p > highest_p: ", highest_p_vars)
            return p, M, k, a, Nexc, good_fit_p
        else:
            #print("Estimation:")
            return highest_p_vars
...
def pgpdML(y,x0,N,Nexc,Padth=0.05):
    ...
    if (not np.isnan(parmhat[0])) and (not np.isnan(parmhat[1])):
        ...
        # compute a p-value even if the good-fit-test is not satisfied
        #if Pad > Padth:
...
def _nll(parmhat,z):
    ...
    #print("a: %s, k: %s, n: %s, m: %s, -L: %s" % (str(a),str(k),str(n),str(m),str(-L)))
